...
For an example of this approach, see the implementation of the [`delay`](@ref "Synth.delay")
and [`tap`](@ref "Synth.tap") signals. The delay line is modeled as a pass-through signal
...
## How to time limit an infinite extent signal like [`oscil`](@ref "Synth.oscil")?

Several signals like [`oscil`](@ref "Synth.oscil") are in principle infinitely extended
...
A plain way to limit the duration of a `oscil` is to use [`clip`](@ref "Synth.clip") like
...
or an [`adsr`](@ref "Synth.adsr") to the amplitude like this -- `oscil(adsr(1.0f0, 0.4),
...
class citizens, though there is some support. See [`stereo`](@ref "Synth.stereo")
...
- [`stereo`](@ref "Synth.stereo") turns a pair of monophonic signals into a stereo signal.
- [`left`](@ref "Synth.left") and [`right`](@ref "Synth.right") fetch the L and R components of a stereo signal.
