...
[`Synth.Signal`](@ref). In this basic tutorial, we'll construct
...
The [`phasor`](@ref "Synth.phasor") and [`oscil`](@ref "Synth.oscil") are two of the most
...
2. Play a middle C note at 0.25 amplitude for 2.0 seconds.
   ...
   Here, [`midi2hz`](@ref "Synth.midi2hz") is used to convert a MIDI note number
...
3. Play a harsh sawtooth wave.
   ...
   The [`phasor`](@ref "Synth.phasor") goes from 0.0 to 1.0 at the given frequency
...
For example, to use a bandpass filter ([`bpf`](@ref "Synth.bpf")) on a noise signal,
...
[`lpf`](@ref "Synth.lpf"), [`bpf`](@ref "Synth.bpf"), [`bpf0`](@ref "Synth.bpf0") and [`hpf`](@ref "Synth.hpf").
...
with varying or fixed tempos using [`clock`](@ref "Synth.clock") and [`Synth.clock_bpm`](@ref). The
...
A [`control`](@ref "Synth.control") is a signal whose value can be updated in near realtime.
...
a separate [`fanout`](@ref "Synth.fanout") operator that wraps such processes with the ability
...
support for stereo signals through [`pan`](@ref "Synth.pan") and [`stereo`](@ref "Synth.stereo"). 
...
1. Stereo signal processes are represented by the `Stereo{L,R}` type. You turn
   two mono signal processes into a stereo signal process using [`stereo`](@ref "Synth.stereo").
...
3. If you have a stereo signal, you can get at its left and right channels as
   mono signals using [`left`](@ref "Synth.left") and [`right`](@ref "Synth.right").

4. You can mix down a stereo to a mono signal using [`mono`](@ref "Synth.mono") which will
   add the left and right channels in the simplest case, but also accepts a "panner"
   signal for more complex mixing.

5. [`render`](@ref "Synth.render") supports rendering stereo signals.
